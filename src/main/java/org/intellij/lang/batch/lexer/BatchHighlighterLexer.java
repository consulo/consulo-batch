...
public class BatchHighlighterLexer extends LayeredLexer
{
	public BatchHighlighterLexer()
	{
		...
		registerLayer(new FlexAdapter(stringLexer), BatchTokenTypes.STRING_LITERAL);
		...
		registerLayer(new FlexAdapter(extressionLexer), BatchTokenTypes.EXPRESSION);
		...
		registerLayer(new FlexAdapter(labelLexer), BatchTokenTypes.LABEL_REFERENCE);
	}
}
